= Crear un NIF (Native Implemented Function) para AtomVM

En esta sección, se explicará cómo crear una extensión simple en C para implementar la función "add" en AtomVM. La estructura del proyecto será la siguiente:
...
Una vez que el componente del NIF esté listo, se debe incluir en el directorio `src/platforms/esp32/components` de AtomVM. Al colocarlo allí, se incorporará automáticamente durante el proceso de compilación, siguiendo los pasos detallados en la página xref:atomvm:devenv.adoc#compilar-atomvm[Entorno de Desarrollo].

Es crucial ejecutar el comando `idf.py reconfigure` antes de `idf.py build` para que el ESP-IDF detecte y reconozca el nuevo componente.
