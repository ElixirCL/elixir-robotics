defmodule Auto1 do
  ...
  def start do
    ...
    case connect_wifi_with_retries(@wifi_max_retries) do
      ...
      {:error, reason} ->
        ...
        # Fallback: levantar un AP propio para que al menos puedas controlarlo desde el celular.
  ...
  defp start_ap_and_wait do
    ...
    # Levantamos AP y dejamos que el servidor HTTP escuche normalmente.
    # En ESP-IDF el IP del AP suele ser 192.168.4.1.
...
defmodule Auto1.Page do
  def html do
    """
    <!DOCTYPE html><html><body style="text-align:center; background:#111; color:#fff;">
    <h1>Auto1 Controller</h1>
    <p>Use this page from any phone in the same WiFi network.</p>
    <button onclick="fetch('/cmd?move=forward')" style="height:100px; width:100px;">⬆️</button><br>
    <button onclick="fetch('/cmd?move=left')" style="height:100px; width:100px;">⬅️</button>
    <button onclick="fetch('/cmd?move=stop')" style="height:100px; width:100px; background:red;">🛑</button>
    <button onclick="fetch('/cmd?move=right')" style="height:100px; width:100px;">➡️</button><br>
    <button onclick="fetch('/cmd?move=backward')" style="height:100px; width:100px;">⬇️</button>
    </body></html>
    """
  end
end
